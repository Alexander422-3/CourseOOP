...
class Menu:
    ...
    def field(self):
        ...
        button_start_game = tk.Button(self.parent, text="Начать игру", font=("Arial", 12), command=self.start_game,
                                      bg="white")
        button_start_game.place(relx=0.35, rely=0.35, anchor="center", width=140, height=50)
        button_rules_game = tk.Button(self.parent, text="Правила игры", font=("Arial", 12), command=self.rules_game,
                                      bg="white")
        button_rules_game.place(relx=0.66, rely=0.35, anchor="center", width=140, height=50)
        button_quit_game = tk.Button(self.parent, text="Выход из игры", font=("Arial", 12), command=self.quit_game,
                                     bg="white")
        button_quit_game.place(relx=0.5, rely=0.75, anchor="center", width=140, height=50)
        button_registration = tk.Button(self.parent, text="Регистрация", font=("Arial", 12), command=self.reg,
                                        bg="white")
    ...
    def reg(self):
        ...
        self.nickname_pole = tk.Entry(reg_game, width=5, font=("Arial", 12), justify="center",
        ...
        self.successful_label = tk.Label(reg_game, text="", font=("Arial", 11),
                                         bg="#6361AD", foreground="white")
        self.successful_label.place(relx=0.5, rely=0.8, anchor="center", width=160, height=25)
    ...
    def auto(self):
        ...
        self.nickname_pole = tk.Entry(auth_game, width=5, font=("Arial", 12), justify="center",
                                      highlightbackground="black",
                                      highlightthickness=1)
        self.nickname_pole.place(relx=0.5, rely=0.15, anchor="center", width=150, height=25)
...
class Game:
    ...
    def widgets(self):
        sdvig_x = 0
        for i in range(7):
            square = tk.Canvas(self.master, width=68, height=68, bg="white",
                               highlightbackground="black", highlightthickness=1)
            square.grid(row=0, column=i, padx=17, pady=90)
            self.squares.append(square)

        for y in range(7):
            user_square = tk.Entry(self.master, width=5, font=("Arial", 12), justify="center",
                                   highlightbackground="black",
                                   highlightthickness=1)
            user_square.place(relx=0.071 + sdvig_x, rely=0.55, anchor="center", width=71, height=71)
        ...
        start_button = tk.Button(self.master, text="Запустить игру", font=("Arial", 12), command=self.start_game,
                                 bg="white")
        start_button.place(relx=0.5, rely=0.7, anchor="center", width=120, height=30)

        self.show_answer_button = tk.Button(self.master, text="Показать ответ", font=("Arial", 12),
                                            command=self.print_numbers_sequence, bg="white")
        ...
        self.button_hide_answer = tk.Button(self.master, text="Cкрыть ответ", font=("Arial", 12),
                                            command=self.clear_squares_text, bg="white")
        ...
        self.quit_button = tk.Button(self.master, text="Выход в меню", font=("Arial", 12), command=self.on_closing, bg="white")
        self.quit_button.place(relx=0.1, rely=0.95, anchor="center", width=120, height=30)

        self.answer_button = tk.Button(self.master, text="Ответить", font=("Arial", 12), command=self.check_answer,
                                       bg="white")
        self.answer_button.place(relx=0.5, rely=0.8, anchor="center", width=120, height=30)
        self.answer_button.config(state=DISABLED)

        game_title = tk.Label(self.master, text="Запомни число", font=("Arial", 20), bg="#6361AD", foreground="white")
        game_title.place(x=260, y=5)
        self.sequence_label = tk.Label(self.master, text="", bg="#B2B1CF")

        self.game_level = tk.Label(self.master, text="Уровень " + str(self.level), font=("Arial", 14), bg="#6361AD",
                                   foreground="white")
        self.game_level.place(x=310, y=50)

        self.show_stats_button = tk.Button(self.master, text="Cтатистика", font=("Arial", 12),
                                           command=self.show_all_stats,
                                           bg="white")
        self.show_stats_button.place(relx=0.1, rely=0.85, anchor="center", width=120, height=30)
    ...
    def show_random_number(self):
        if self.current_square_index < len(self.squares):
            ...
            square.delete("number")
            square.create_text(34, 34, text=str(random_number), font=("Arial", 12), tags="number")
            self.k += 1
    ...
    def print_numbers_sequence(self):
        ...
        for i in range(0, 7):
            ...
            square_answers.create_text(34, 34, text=str(self.numbers_sequence[i]), font=("Arial", 12),
                                       tags="number")
            index += 1
    ...
    def get_answer(self):
        if all(answer == -1 for answer in self.user_squares):
            return 1
        for i in range(7):
    ...
    def check_answer(self):
        ...
        if self.get_answer() == 1:
            return 1
        for i in range(7):
            answers[i] = int(self.user_squares[i].get())
        self.history[self.level] = {'program_number': self.numbers_sequence, 'user_answer': answers}
        if self.level == 4 and self.user_answers == self.numbers_sequence:
            ...
            result = "Уровень пройден успешно!"
            self.level += 1
            self.game_level.config(text="Уровень " + str(self.level))
            self.clear_squares_text()
            self.clear_user_squares_text()
    ...
    def statistica(self, current_level, result):
        ...
        statistic_button = tk.Button(statistic, text="Cохранить статистику", font=("Arial", 12),
                                     command=lambda: self.save_stats(statistic, current_level),
                                     bg="white")
    ...
    def show_all_stats(self):
        ...
        statistic_frame.pack(fill=tk.BOTH, expand=True)

        scrollbar = tk.Scrollbar(statistic_frame, orient=tk.VERTICAL)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        statistic_canvas = tk.Canvas(statistic_frame, width=400, height=600, yscrollcommand=scrollbar.set,
                                     background="#6361AD")
        statistic_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        scrollbar.config(command=statistic_canvas.yview)

        statistic_frame_inner = tk.Frame(statistic_canvas, width=400, height=600, background="#6361AD")
        statistic_canvas.create_window((0, 0), window=statistic_frame_inner, anchor="nw")

        statistic_label1 = tk.Label(statistic_frame_inner, text="Cтатистика", font=("Enter", 20),
                                    bg="#6361AD", foreground="white", width=10, height=1)
        statistic_label1.grid(row=0, column=0, columnspan=3, pady=10)
        statistic_label2 = tk.Label(statistic_frame_inner, text="Игрок", font=("Enter", 18),
                                    bg="#6361AD", foreground="white", width=8, height=1)
        statistic_label2.grid(row=1, column=0, pady=10)
        statistic_label3 = tk.Label(statistic_frame_inner, text="Уровень", font=("Enter", 18),
                                    bg="#6361AD", foreground="white", width=8, height=1)
        statistic_label3.grid(row=1, column=1, pady=10)
        statistic_label4 = tk.Label(statistic_frame_inner, text="Место", font=("Enter", 18),
                                    bg="#6361AD", foreground="white", width=8, height=1)
        ...
        for i in range(len(users)):
            temp = []
            label1 = tk.Label(statistic_frame_inner, text=users[i]["nickname"], font=("Enter", 18),
                              bg="#6361AD", foreground="white", width=8, height=1)
            label1.grid(row=2 + i, column=0, pady=5)
            label2 = tk.Label(statistic_frame_inner, text=users[i]["maxlevel"], font=("Enter", 18),
                              bg="#6361AD", foreground="white", width=8, height=1)
            label2.grid(row=2 + i, column=1, pady=5)
            label3 = tk.Label(statistic_frame_inner, text=str(i + 1), font=("Enter", 18),
                              bg="#6361AD", foreground="white", width=8, height=1)
        ...
        statistic_frame_inner.update_idletasks()

        statistic_canvas.config(scrollregion=statistic_canvas.bbox("all"))

        show_statistic.mainloop()
    ...
    def show_history(self):
        ...
        for level, data in self.history.items():
            ...
            program_label = tk.Label(history_window, text=f"Числа программы: {data['program_number']}")
            program_label.pack()
